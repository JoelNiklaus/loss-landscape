# resnet20 diufpc
nohup python plot_surface.py --model resnet20 --x=-1:1:51 --y=-1:1:51 \
...
# resnet56 sgd
python plot_surface.py --model resnet56 --x=-1:1:51 --y=-1:1:51 \
...
# densenet 121
python plot_surface.py --model densenet121 --x=-1:1:51 --y=-1:1:51 \
...
nohup python plot_surface.py --model densenet121 --x=-1:1:51 --y=-1:1:51 \
--cuda --ngpu 4 --threads 8 --batch_size 4096 \
--model_file datasets/cifar10/trained_nets/densenet121_sgd_lr=0.1_bs=64_wd=0.0005/model_300.t7 \
--dir_type weights --xnorm filter --xignore biasbn --ynorm filter --yignore biasbn --plot > nohup.out &

# init baseline vgg like locally without cuda
python plot_surface.py --model init_baseline_vgglike --dataset cinic10 --x=-1:1:51 --y=-1:1:51 \
...
# init baseline vgg like on diufpc
